...
def dc_form(request):
    ...
    if not username:
        return redirect('/')
    return render(request, 'dc_form.html', {
        'username': username,
        'view_mode': False,
        'states': [],
        'cities': []
...
def delivery_challan_view(request, dc_id):
    ...
    try:
        with connection.cursor() as cursor:
            ...
            cursor.execute("""
                SELECT item_name, description, uom, quantity, weight, square_feet, rate_per_each,
...
def delivery_challan_list(request):
    """List delivery challans with status PENDING for the authenticated user"""
    username = request.session.get('username', '')
    if not username:
        return redirect('/')
    try:
            ...
            with connection.cursor() as cursor:
                ...
                cursor.execute("""
                    INSERT INTO dc_details (
                        ...
                        total_dispatch_quantity, total_rate, project_name, project_incharge,
